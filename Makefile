...
jupyter: # Run jupyterlab on statapp service
	...
	@docker-compose exec statapp_service sh -c "uv run jupyter lab --ip=0.0.0.0 --allow-root" || true ;
...
mlflow: # Run Mlflow Tracking User Interface on statapp service
	...
	@docker-compose exec statapp_service sh -c "uv run mlflow server --backend-store-uri sqlite:////statapp/mlflow.db --host 0.0.0.0 --port 5000" ;
